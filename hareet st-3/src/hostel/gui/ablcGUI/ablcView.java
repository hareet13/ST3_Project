package hostel.gui.ablcGUI; 

import hostel.hstlmodels.ablcmodels;
import hostel.hstlservices.ablcservices;
...
public class ablcView extends JPanel {
	...
	public ablcView() {
		...
		table.setRowHeight(24);				/// set all row height must needed for error free printing 
		scrollPane.setViewportView(table);
		
		
		JLabel lblNewLabel = new JLabel("Ablc Table");
		...
		cbStudId.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(!cbStudId.getSelectedItem().equals("-Select-")) {
					...
					/*if( cbStudId.getSelectedItem().equals("-Select-"))rsort.setRowFilter(null);	
					else */
						tableDispl(i,"","");///----calling search method
		...
		///-------------------------------------Print current table view or searched According to student id

		
		btnPrintStud = new JButton("Print ");
		btnPrintStud.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
				 ...
				 printCompleteMsg(complete);///method for Print Complete or cancel info
				}
		...
		btnMnthOk = new JButton("Ok");
		btnMnthOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				final String mnth=monthName(monthChooser.getMonth());//-----------calling method to get month name..
				 
				tableDispl(0,"", mnth);///----calling Jtable filter method of Filtering or Searching
				 
				btnMnthprint.addActionListener(new ActionListener() {////action listner of Month Print btn
					public void actionPerformed(ActionEvent arg0) {
						try {
							...
							printCompleteMsg(cmplete);//method for Print Complete or cancel info
						}
	...
	/////============----------------Ok button for search and printing fees with Particular year
		btnSerchYear = new JButton("Ok");
		...
		panelSerhYear.add(btnPrintYear);
						...
						///=============Radio Button Print Particular Month Fees
		...
		add(rbSearch2);
										///=============Radio Button Print Particular Student Fee
		...
		add(rbSearch1);
					...
					//-----------------Method for Filter view or search with string or number of current table view
		public void tableDispl(int s,String yer,String m) {
			//System.out.println(s+yer+m); 
		 	try {
		 		if(s!=0)rsort.setRowFilter(RowFilter.numberFilter(ComparisonType.EQUAL, s ,1));/// filtering using numbers // -- the argument '1' is the column number to be sorted
				if (!m.equals("")) {
	...
	////-----------------------method for Print Complete or cancel Information 	
