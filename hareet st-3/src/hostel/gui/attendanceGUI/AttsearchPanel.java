...
public class AttsearchPanel extends JPanel {
	...
	public AttsearchPanel() {
		...
		rb1AttsearchID.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try{
					...
					///---- filtering jtable using numbers
					if(id!=0) {
						try {
							tablRsrt.setRowFilter(RowFilter.numberFilter(ComparisonType.EQUAL,id, 3));// -- the argument '3' is the 'column number'(0 to 3) to be sorted
							Viewtable.setRowSorter(tablRsrt);
		...
		btnDateSearchOk = new JButton("Ok");				///------------search attendance with date with date
