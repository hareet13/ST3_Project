package hostel.gui.attendanceGUI;

import hostel.hstlmodels.attModel; 
import hostel.hstlservices.atendanService;
...
public class hstlattendgui extends JPanel {
	...
	public hstlattendgui() {
		...
		Search_panel.setLayout(null);
		 ...
		 as=new AttsearchPanel();
		ScrollattSearch = new JScrollPane(as);//----to add view panel class into main attendance
		...
		JButton btnNewButton = new JButton("Display Students");///---------------button for displaying students
		...
		JButton btnNewButton_1 = new JButton("Add Absent ->");// ------------------------------add the absentees
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				} catch (Exception e) {// exception occoures when presentJlist is 'null' so only check in display jlist
		...
		JButton btnNewButton_2 = new JButton("<- Remove Absent");//-----remove selected item from Absent list
		...
		subm_Att = new JButton("Submit");			//////******************************************submit attendance************
		subm_Att.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				
				Date dt=dateChooser.getDate();	//////////					-------get date from date chooser
				String sd=String.format("%1$td-%1$tm-%1$tY",dt);
				
				lst2mdl=(DefaultListModel)AbsntList.getModel();		//--------getting current absentees 'list' model
				lst3mdl=(DefaultListModel)prsntList.getModel();		//--------getting current Presents 'list' model
				
				//--------------------------------------- Initialising absentees and presents storage name and id
				...
				String st4[]=null,st5[]=null;		//---for temporarily storing splitted value from current absentees 'list' model and from general list mdl
				int k = 0;							//-----------	increment for present and absent
				
				for(int i=0;i<absid.length;i++) {		//---get absentees list
					//st4=((String) lst2mdl.elementAt(i))).replaceAll("\\D+","");    ///---------getting only integer values from string
					 st4=(String.valueOf(lst2mdl.elementAt(i)).split("\\s+"));       ///-----split strings with space of absentees
				...
				k=0;
				for(int i=0;i<prsntid.length;i++) {		//----get presents list
					//st5=((String) lst2mdl.elementAt(i))).replaceAll("\\D+","");		/////---------getting only integer values from string
					 st5=(String.valueOf(lst3mdl.elementAt(i)).split("\\s+"));			/////-----split strings with space of presents
					 //JOptionPane.showMessageDialog(null, st5[0]+"  "+st5[1]);
				...
				try {
					...
					am.setMnth(monthName(Integer.parseInt(date[1])));/////-------getting month name from pgmr defined method
		...
		JButton button = new JButton("Clear All"); ///----------------------  to clear all data
		...
		AddprsntBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				}catch (Exception e1) {// exception occoures when presentJlist is 'null' so only check in display jlist
					//JOptionPane.showMessageDialog(null, "hai");
		...
		absntAddAll.addActionListener(new ActionListener() {//------------to add all students to Absent jlist
			public void actionPerformed(ActionEvent arg0) {
