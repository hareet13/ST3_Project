...
import hostel.hstlmodels.visitormodels;
...
public class visitorservices {
	...
	public ComboBoxModel vistComb(){
		try{
			PreparedStatement ps=con.prepareStatement("select distinct stdname from hstlstud order by stdname ");
	...
	public void vistIns(visitormodels vm){
		try{
			...
			PreparedStatement ps=con.prepareStatement("select vist_seri from hstlvstr where vistname like ? and vistph like ? and viststudnam like ? and vistrelat like ? and  vistdate like ? and enterytm like ? and  leavtm like ? and vistpurps like ?");
			...
			if(rs.next()) JOptionPane.showMessageDialog(null, "Entered record Already Exist....!");
				...
				ps=con.prepareStatement("insert into hstlvstr(vistname,vistph,viststudnam,vistrelat,vistdate,enterytm,leavtm,vistpurps)values(?,?,?,?,?,?,?,?)");
				 ps.setString(1, vm.getVstnam());
				 ps.setLong(2,vm.getVstph());
				 ps.setString(3,vm.getVstudnam());
				 ps.setString(4,vm.getVstrelat());
				 ...
				 System.out.println(ps);
				ps.executeUpdate();
				JOptionPane.showMessageDialog(null, "One Visitor Record Inserted...!");
			}
		}
		catch(SQLException se){
	...
	public TableModel vistTable(){
		...
		try{
			PreparedStatement ps=con.prepareStatement("select * from hstlvstr");
			ResultSet rs=ps.executeQuery();
	...
	public visitormodels vistOk(visitormodels vm) {
		try {
			...
			PreparedStatement ps=con.prepareStatement("select vistname,vistph,viststudnam,vistrelat,vistdate,leavtm,vistpurps from hstlvstr where vist_seri=?");
	...
	public void vistDel(visitormodels vm){
		try {
			...
			if(c==0) {
				PreparedStatement ps=con.prepareStatement("delete from hstlvstr where vist_seri=?");
				ps.setInt(1,vm.getVstslno());
				ps.executeUpdate();
	...
	public void vistUpdate(visitormodels vm) {
		...
		if(c==0) {
			try {
				PreparedStatement ps=con.prepareStatement("select vist_seri from hstlvstr where vistname like ? and vistph like ? and viststudnam like ? and vistrelat like ? and  vistdate like ? and  leavtm like ? and vistpurps like ?");
				 ps.setString(1, vm.getVstnam());
				 ps.setLong(2,vm.getVstph());
				 ps.setString(3,vm.getVstudnam());
				 ps.setString(4,vm.getVstrelat());
				...
				if(rs.next()) JOptionPane.showMessageDialog(null, "Entered record Already Exist....!");
					...
					ps=con.prepareStatement("update hstlvstr set vistname=?,vistph=?,viststudnam=?,vistrelat=?,vistdate=?,leavtm=?,vistpurps=? where vist_seri=?");
					ps.setString(1, vm.getVstnam());
					 ps.setLong(2,vm.getVstph());
					 ps.setString(3,vm.getVstudnam());
					 ps.setString(4,vm.getVstrelat());
