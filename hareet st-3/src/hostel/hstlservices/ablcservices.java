...
public class ablcservices {
	...
	//////-------------------------------combo Student id taken from hstlgrand
	public ArrayList<String> getStUdid( ){
		ArrayList<String> sid=new ArrayList<String>();
		try {
			sid.add("-Select-");
			PreparedStatement ps=con.prepareStatement("select grndstudid from hstlgrand order by grndstudid");
...
////////////////////////////--------Fetching student details
	public ablcmodels ablcStdinfo(ablcmodels am) {
		try{
			PreparedStatement ps=con.prepareStatement("select grndblknam,grndflrnam,grndrmnum,grndrmrent from hstlgrand where grndstudid=?");
			...
			ps=con.prepareStatement("select stdname from hstlstud where std_id=?");
			ps.setInt(1,am.getAblcstdId());
			rs=ps.executeQuery();
		...
		return null; 		
	}///end of Fetching student details
	
	///////---------------------------------------Saving Ablc Values to Data base
	int sid;
