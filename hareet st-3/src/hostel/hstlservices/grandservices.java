...
import hostel.hstlmodels.grandmodels;
...
public class grandservices {
	...
	public ComboBoxModel grndstudcbId() {
		...
		try {
			PreparedStatement ps=con.prepareStatement("SELECT distinct std_id FROM hstlstud order by std_id ");
			ResultSet rs=ps.executeQuery();
			Vector<Object> sid=new Vector<Object>();
			
			sid.add("-Select-");
			if(rs.isBeforeFirst()&&rs.getRow()==0){// <==== checking if table row is not empty
				while(rs.next()) {
					int i=1;
					sid.add(rs.getInt(i));
					//System.out.println( "Student id from grand service"+rs.getInt(i));
					i++;
				}
			}
			/*rs.close();
	...
	///--------------------fetching student name and parent name-------------------------
	public grandmodels getstudnames(grandmodels gm) {
		try {
			PreparedStatement ps=con.prepareStatement("SELECT  stdname,stdparnam,stdstat FROM hstlstud where std_id=?");
...
///////=======================---------------------------------grand combo box getting block names--------------
	public ComboBoxModel grndbloknam() {
		System.out.println( "block combo box name from grand service");
		try {
			PreparedStatement ps=con.prepareStatement("SELECT distinct blokname from hstlblok order by blokname ");
			ResultSet rs=ps.executeQuery();
			...
			if(rs.isBeforeFirst()&&(rs.getRow()==0)){
				while(rs.next()) {
					...
					//System.out.println( "Student id from grand service"+rs.getString(i));
					i++;
				}
			}
			/*rs.close();
	...
	///--------------------fetching floor name combo box items  -------------------------
		...
		public ArrayList<String> getfloornums(grandmodels gm) {
			...
			try {
				...
				ps=con.prepareStatement("SELECT blokstatus from hstlblok where blokname=?");
				ps.setString(1,gm.getGrndBlkname());
				 rs=ps.executeQuery();
				 if(rs.isBeforeFirst()&&(rs.getRow()==0)){
					while(rs.next()) {
		...
		//===================================--------------------------------Grand Submit services	
			public void grndSubmit(grandmodels gm){
				try {	//checking if same data is there					 			   
					PreparedStatement ps=con.prepareStatement("select grand_id from hstlgrand where grndstudid like ? and grndblknam like ? and grndflrnam like ? and grndrmnum like ? and grndrmrent like ? and grndstat like ?");
							ps.setInt(1, gm.getGrndstudId());
						...
						ResultSet rs=ps.executeQuery(); 
						 ...
						 else {	///checking student already exists or not.....
						   ps=con.prepareStatement("select grndstudid from hstlgrand where grndstudid like ?");
						    ps.setInt(1,gm.getGrndstudId());
						   rs=ps.executeQuery();
						   if(rs.next()) {///if yes return
							   JOptionPane.showMessageDialog(null, "Already Granded to student...!");
							   return;
						   }
						   else {/// else insert new grand record.
								   ps=con.prepareStatement("insert into hstlgrand(grndstudId,grndblknam,grndflrnam,grndrmnum,grndrmrent,grndstat)values(?,?,?,?,?,?)");
									ps.setInt(1, gm.getGrndstudId());
									ps.setString(2,gm.getGrndBlkname());
									ps.setString(3, gm.getGrndFlornum());
									ps.setString(4,gm.getGrndRoomnum());
		...
		/////-------------------------------Granded table view--------------------------------------------------
			...
			public TableModel grandTable(){
				...
				try{
					PreparedStatement ps=con.prepareStatement("select * from hstlgrand");
					ResultSet rs=ps.executeQuery();
	...
	//////---------------------------------------Grand Id Submitting ok button
			public grandmodels grandOk(grandmodels gm) {
				try {
					PreparedStatement ps=con.prepareStatement("select grndstudid,grndblknam,grndflrnam,grndrmnum,grndrmrent,grndstat from hstlgrand where grand_id=?");
			 ...
			 if(rs.next()) {
				 ...
				 return;
					...
					int c=JOptionPane.showConfirmDialog(null,"Are you sure to Update a Granded Record","Confirm",JOptionPane.YES_NO_OPTION);
						if(c==0) {
							  ps=con.prepareStatement("UPDATE  hstlgrand SET grndstudid=?,grndblknam=?,grndflrnam=?,grndrmnum=?,grndrmrent=?, grndstat=? WHERE  grand_id=? ");
								ps.setInt(1,gm.getGrndstudId());
								ps.setString(2,gm.getGrndBlkname());
								ps.setString(3,gm.getGrndFlornum());
								ps.setString(4, gm.getGrndRoomnum());
