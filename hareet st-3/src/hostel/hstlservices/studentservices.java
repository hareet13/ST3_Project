...
import hostel.hstlmodels.floormodels;
import hostel.hstlmodels.studentmodels;
...
public class studentservices {
	...
	public DefaultTableModel studView(){
		try{
			PreparedStatement ps=con.prepareStatement("select * from hstlstud where 1");//to see in student alphabetical order 
...
//===================================================================method for ADDING OR submitting Student Record===================
	public void  addStud(studentmodels sm){
		try{
			PreparedStatement ps=con.prepareStatement("select std_id from hstlstud where stdname like ? and stdgend like ? and stddob like ? and stdaddr like ? and stdph like ? and stdemail like ? and stdparnam like ? and stdparph like ? and stdparem like ? and stdstat like ? ");
		  ...
		  if(rs.next()){
			  ...
			  ps=con.prepareStatement("insert into hstlstud(stdname,stdgend,stddob,stdaddr,stdph,stdemail,stdparnam,stdparph,stdparem,stdstat)values(?,?,?,?,?,?,?,?,?,?)");
			    ps.setString(1,sm.getStudNam());
	        	ps.setString(2,sm.getStudGend());
			    ps.setString(3,sm.getStdDob());
			    ps.setString(4,sm.getStudAddr());
	...
	public studentmodels editidOk(studentmodels sm){
		try{
			...
			PreparedStatement ps=con.prepareStatement("select stdname,stdgend,stddob,stdaddr,stdph,stdemail,stdparnam,stdparph,stdparem,stdstat from hstlstud where  std_id=?");
			...
			if(!rs.isBeforeFirst()&&rs.getRow()==0){
				...
				while(rs.next()){
					sm.setStudNam(rs.getString(1));	///System.out.println(sm.getStudNam());
					sm.setStudGend(rs.getString(2));//System.out.println(sm.getStudGend());
					sm.setStdDob(rs.getString(3));//System.out.println(sm.getStdDob());
					sm.setStudAddr(rs.getString(4));//System.out.println(sm.getStudAddr());
					sm.setStudPh(rs.getLong(5));//System.out.println(sm.getStudPh());
					sm.setStudEml(rs.getString(6));//System.out.println(sm.getStudEml());
					sm.setParntname(rs.getString(7));//System.out.println(sm.getParntname());
					sm.setParPh(rs.getLong(8));//System.out.println(sm.getParPh());
					sm.setParEml(rs.getString(9));//System.out.println(sm.getParEml());
					sm.setStudStat(rs.getString(10));//System.out.println(sm.getStudStat());
		...
		try{
			if(x==0){
				...
				if(rs.next())JOptionPane.showMessageDialog(null, "Record already exists....");///if record exists then print message.
					...
					ps=con.prepareStatement("UPDATE  hstlstud SET  stdname=?,stdgend=?,stddob=?,stdaddr=?,stdph=?,stdemail=?,stdparnam=?,stdparph=?,stdparem=?,stdstat=? WHERE std_id=?");
				     ps.setString(1,sm.getStudNam());
		        	 ps.setString(2,sm.getStudGend());
				     ps.setString(3,sm.getStdDob());
				     ps.setString(4,sm.getStudAddr());
