...
public class atendanService {
	...
	//----------------------select students to display in jlist and to submit attendance 
	public  DefaultListModel selectStuds() {
		try {
			DefaultListModel dlm=new DefaultListModel();
			  ps=con.prepareStatement("select distinct std_id,stdname from hstlstud where stdstat='Active'");
			  rs=ps.executeQuery();
			while(rs.next()) {
		...
		return null;
										...
										//--------------submit or insert attendance into database
	public int insertAtt(attModel am) {
		...
		int f=0;	//--flag used for indicating success of insert
		
						//---------to insert Presents list
		for(int i=0;i<prsntid.length;i++) { 
			try {
				  ps=con.prepareStatement("insert into hstlatt(`day`,`mnth`,`year`,`stdid`,`stdname`,`attStat`)values(?,?,?,?,?,?)");
		...
		if(f==1) return 1;      // insert success...!
		else return 0;	//--- insert failure
	}//----------------------------------------------end of method insertAtt
	
										///---------------------to View All Attendance Table Datas------------------------ 
	...
	public DefaultTableModel ViewAttend(){
		 try {
			...
			ps=con.prepareStatement("SELECT * FROM `hstlatt`");
			rs=ps.executeQuery();
		...
		return null;
	}//--------------------------------------------end of ViewAttend method 
}//-------end of class atendanService
